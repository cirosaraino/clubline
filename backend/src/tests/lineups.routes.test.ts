...
import { createLineupsRouter } from '../routes/lineups.routes';
...
test('PUT /lineups/:id/players creates club-wide lineup notifications and publishes notifications scope', async () => {
  ...
  const router = createLineupsRouter({
    db: db as any,
  ...
  await withTestServer(router, async (baseUrl) => {
    const response = await fetch(`${baseUrl}/100/players`, {
      method: 'PUT',
      headers: {
  ...
  assert.deepEqual(published, [
    {
      scopes: ['lineups', 'attendance', 'notifications'],
