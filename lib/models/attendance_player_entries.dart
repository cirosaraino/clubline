...
class AttendancePlayerEntries {
  ...
  int completedDaysCount(Iterable<DateTime> dates) {
    return dates.where((date) => !(entryForDate(date)?.isPending ?? true)).length;
  }

  static List<AttendancePlayerEntries> groupEntries(List<AttendanceEntry> sourceEntries) {
    final grouped = <dynamic, List<AttendanceEntry>>{};
    final playersById = <dynamic, PlayerProfile?>{};

    for (final entry in sourceEntries) {
      grouped.putIfAbsent(entry.playerId, () => []).add(entry);
